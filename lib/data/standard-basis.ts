...
export const standardBases: StandardBasis[] = [
  ...
  row(
    ...
    ],
    {
      ...
      usedInCalculation: "ΔV는 계산 구간 편도 L. 표 232.3-1 가산은 인입구→기기 경로 길이. 구간=경로일 때만 ΔV%와 허용 참고값을 비교. 혼합부하는 표 숫자를 고르지 않음. % 분모는 사용자 기준전압(KEC 공식 분모 미확정)",
  ...
  row(
    ...
    ["kec", "iec"],
    "standard",
    "S = (I/k)√t. k는 사용자가 표에서 입력.",
    "단열식 기반 계산. k값 및 적용조건은 별도 확인 필요",
    [
      "표 142.3-1과 k 수치표를 내장하지 않습니다.",
      "KEC 142.3.2는 표 142.3-1 또는 계산식으로 선정합니다. Ampory는 계산식만 제공합니다.",
      "t > 5 s이면 단열식 결과를 표시하지 않습니다. 보호도체 선정이 불가능한 것이 아니라 이 계산식의 적용범위 밖입니다.",
    ],
    {
      domesticReview: "KEC 142.3.2 보호도체 최소 단면적",
      relatedStandards: ["KS C IEC 60364-5-54"],
      usedInCalculation: "t ≤ 5 s일 때만 S = (I/k)√t (사용자 k). t > 5 s는 결과 숨김",
      referenceOnly: ["표 142.3-1 수치는 사용자가 원문에서 확인", "기계적 최소단면적은 설치조건 입력 후 별도 검토"],
    },
  ),
